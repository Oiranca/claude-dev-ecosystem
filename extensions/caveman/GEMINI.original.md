...
Caveman makes AI coding agents respond in compressed, caveman-style prose — cutting ~65-75% of output tokens while keeping full technical accuracy. It ships as a Gemini CLI plugin, a Gemini CLI plugin, a Gemini CLI extension, and as agent rule files for Cursor, Windsurf, Cline, Copilot, and 40+ others via `npx skills`.
...
3. Checks `~/.claude/settings.json` for an existing statusline config; if missing, appends a nudge telling Gemini CLI to offer setup on first interaction
...
| Gemini CLI | Plugin in `plugins/caveman/` with `hooks.json` | Yes — SessionStart hook |
...
`llm_run.py` calls `claude -p --system-prompt ...` per (prompt, arm), saves output to `evals/snapshots/results.json`. `measure.py` reads the snapshot offline with tiktoken (OpenAI BPE — approximates Gemini CLI's tokenizer, ratios are meaningful, absolute numbers are approximate).
...
`benchmarks/` runs real prompts through the Gemini CLI API (not Gemini CLI CLI) and records raw token counts. Results are committed as JSON in `benchmarks/results/`. The benchmark table in README is generated from these results — update it when regenerating.
