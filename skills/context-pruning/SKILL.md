...
- **Coordination**: Typically executed by the **context-manager** or the **product-manager** (Team Lead) to prepare the Shared Task List.
- **Skip Conditions**:
...
Also check `.agent-cache/artifact_freshness.json` when present to verify ownership and timestamps.
...
## Expected Output (Communication to Shared Task List)
