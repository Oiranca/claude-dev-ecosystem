...
This skill reduces wasted reads and keeps multi-agent cycles efficient.
...
- Cost class: CHEAP
- Run before any agent that would otherwise start with broad reads.
...
Also check `.agent-cache/artifact_freshness.json` when present.
...
Use targeted searches to find files related to the task:
...
## Handoff Notes
Short guidance for the downstream agent.
