...
- **Cost Class**: MEDIUM.
- **Authorization**: Requires explicit milestone authorization from the **Team Lead** via the Shared Task List.
- **Skip Conditions**:
...
- **Budget & Locks**:
  - Check `.agent-cache/skill_budget_state.json` before rerunning (Max 2 broader validation runs per cycle; max 1 run per skill per cycle).
...
1. **Communicate**: Post the result summary to the **Shared Task List**.
