...
Skills are modular, self-contained folders that extend Gemini CLI's capabilities by providing
...
- "Where should I create this skill? If you do not have a preference, I will place it in `$CODEX_HOME/skills` (or `~/.codex/skills` when `CODEX_HOME` is unset) so Gemini CLI can discover it automatically."
