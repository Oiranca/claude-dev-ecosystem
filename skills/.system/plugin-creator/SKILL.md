---
name: plugin-creator
description: Create and scaffold plugin directories for Gemini CLI with a required `.codex-plugin/plugin.json`, optional plugin folders/files, and baseline placeholders you can edit before publishing or testing. Use when Gemini CLI needs to create a new local plugin, add optional plugin structure, or generate or update repo-root `.agents/plugins/marketplace.json` entries for plugin ordering and availability metadata.
---
...
3. Generate or update the repo marketplace entry when the plugin should appear in Gemini CLI UI ordering:
...
- Treat plugin order in `plugins[]` as render order in Gemini CLI. Append new entries unless a user explicitly asks to reorder the list.
